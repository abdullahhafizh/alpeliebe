...
<form action="<?php echo $own_links;?>/search" method="post" id="form-validated" >
	  <div class="well">
	  		<div class="row">	  			
	  			<div class="col-md-3">
                    <input type="text" name="date_from" id="date_from"  value="<?php echo isset($param['date_from'])?$param['date_from']:'';?>" class="validate[required] form-control datepicker" placeholder="Cari Data Berdasarkan Tanggal Scan Awal" maxlength="16"/>
		  		</div>
	  			<div class="col-md-3">
                    <input type="text" name="date_to" id="date_to"  value="<?php echo isset($param['date_to'])?$param['date_to']:'';?>" class="validate[required] form-control datepicker" placeholder="Cari Data Berdasarkan Tanggal Scan Akhir" maxlength="16"/>
		  		</div>
               ...
               <tbody> 
                ...
                if(count($data) > 0){
                    foreach($data as $r){
                        ...
                        <tr>
                            <td><?php echo ++$no;?></td>
							<td><?php echo myDate($r->time_scan,"d M Y",false);?></td>
                            <td>
...
<!--                            <td><?php echo substr($r->kta_nomor_kartu,0,6)." ".substr($r->kta_nomor_kartu,6,4)." ".substr($r->kta_nomor_kartu,10,6);?></td>-->
                            ...
                            <td>
							<a href="<?php echo $own_links."/detail/?_id="._encrypt($r->kta_id);?>"><span class="label label-default label-form" data-toggle="tooltip" data-placement="top" title data-original-title="See Detail"><li class="fa fa-list-alt"></li></span></a>														
                            </td>							
                        </tr>
                <?php } }?>
