...
<?php js_validate(); ?>
     <form id="form-validated" enctype="multipart/form-data" action="<?php echo $own_links;?>/save_suket" class="form-horizontal" method="post"> 
                    ...
                    <div class="form-group">
						...
						<div class="col-md-6">                                            
                            <div class="input-group">                                         
                                ...
                                <input type="text" name="kta_no_ktp" id="kta_no_ktp"  value="<?php echo isset($val->kta_no_id)?$val->kta_no_id:'';?>" id="kta_no_ktp" class="validate[required]  form-control" placeholder="Masukan No. NIK pada SUKET/ No. SUKET" maxlength="30"/>
                            </div>  
                            <div id="status"></div>
							<span class="help-block"><code>* Wajib Diisi</code></span>						
                       </div>
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body panel-body-table">
                            <div class="row">
                                <div class="col-md-12" style="padding: 10px">
									<div class="form-group ">
										<input type="file" id="upload_lampiran_3" name="upload_lampiran_3" class="form-control" accept="image/*"/>
									</div>
                                </div>
                            </div> 
                        </div>
                    </div>
					<?php if(isset($val) && trim($val->kta_lampiran3)!=""){?>
                    <div class="panel panel-default">
						<div class="panel-body panel-body-table">
							<div class="row">
								<div class="col-md-8" style="padding: 10px">
									<h3>Lampiran SUKET yang Terupload</h3>
									<div class="gallery">
										<a href="<?php echo get_image(base_url()."assets/collections/kta/medium/".$val->kta_lampiran3);?>" title="Image Photo" class="act_modal gallery-item" data-gallery>
												<img alt="" src="<?php echo get_image(base_url()."assets/collections/kta/medium/".$val->kta_lampiran3);?>" width="450">
											<div class="meta">
												<strong>File Utama</strong>
												<span>File hasil dari upload</span>
											</div>
...
//	pic1.src = "<?php echo themeUrl();?>img/loader.gif";
	...
	$(document).ready(function(){
		...
		$('#upload_lampiran_3').bind('change', function() {
			if(this.files[0] != null){
				if(this.files[0].size > 3145728){
				   alert("File terlalu besar. Maksimal ukuran file adalah 3 MB");
				   this.value = '';
		...
		$('#lanjut').click(function() {
		  <?php if(isset($val)){?>
			return true;
		  <?php }else{ ?>
			var l1 			= $("#upload_lampiran_1").val();
			var l2		 	= $("#upload_lampiran_2").val();		
			var l3		 	= $("#upload_lampiran_3").val();		
			if(l1 ==  '' || l2 == '' || l3 == '' ){
...
//				if(usr.length == 16){					
					$("#status").html('<img src="<?php echo themeUrl();?>img/loader.gif" />&nbsp;Cek Daftar No. SUKET...	');
						$.ajax({  
    						type: "POST",  
                            url: URL_AJAX + "/ktp",
    						data: {ktp: usr},
    						success: function(msg){  
    								if(msg == 1) { 
                                        $("#status").html('&nbsp;<img src="<?php echo base_url();?>assets/images/tick.gif" align="absmiddle"> Data SUKET dapat digunakan');
...
/*				}else {
    ...
    $(document).ready(function(){
	    ...
	    $("#upload_lampiran_1,#upload_lampiran_2,#upload_lampiran_3,#upload_foto,#upload_ktp").fileinput({
            showUpload: false,
            showCaption: false,
