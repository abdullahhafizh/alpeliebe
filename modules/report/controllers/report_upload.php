...
class Report_upload extends AdminController {  
	function __construct()    
	{
		...
		$this->_set_title('Report Data Upload');
	...
	function search(){
		...
		if($this->input->post('btn_reset')){
			...
			if($this->jCfg['user']['userrole'] == 33){
				$koordinator = $this->jCfg['user']['id'];
				$operator = "";
			}else{
				$koordinator = $this->jCfg['user']['managerid'];
			...
			$cetak = $this->M->report_upload($par_filter);
...
//			debugCode($this->_v);
			...
			$this->data_table = $this->M->report_upload($par_filter);
			$data = $this->_data(array(
					"base_url"	=> $this->own_link.'/index'
			));
				$data['param'] = array(
