...
class Exp_excel extends AdminController {  
	function __construct()    
	{
		parent::__construct();    
		$this->_set_action();
		$this->_set_action(array("edit","delete"),"ITEM");
		$this->_set_title( 'Export Data' );
	...
	function export_pdf(){
		...
		$par_filter = array("proposer" => $proposer);
		$dt = $this->M->get_excel($par_filter);
//		debugCode($dt);
			...
			foreach((array)$dt['data'] as $k=>$v ){
				...
				$content .= '<tr>
						...
						$content .= '	</td>
									...
									<tr>
										<td align="right" class="masa">'.date('m/Y',strtotime('+1 year',strtotime($v->time_add))).'</td>
									</tr>
								</table>';
	...
	function export_excel(){	
		...
		$data = $this->M->get_excel($par_filter);		
...
*/				
		    $objPHPExcel->getActiveSheet()->setCellValue('A1', 'NO'); // mengisi judul utama
...
//		    $objPHPExcel->getActiveSheet()->setCellValue('A5', 'KECAMATAN : '.$district.' Kode Angka Kecamatan : ( '.str_replace($city_number,'',$district_number).' )'); // mengisi judul kedua
		    $objPHPExcel->getActiveSheet()->setCellValue('B1', 'NPAPG'); // mengisi judul pertama
		    $objPHPExcel->getActiveSheet()->setCellValue('C1', 'NAMA'); // mengisi judul kedua
		    $objPHPExcel->getActiveSheet()->setCellValue('D1', 'ALAMAT / KOTA'); // mengisi judul kedua
		    $objPHPExcel->getActiveSheet()->setCellValue('E1', 'TANGGAL PENERBITAN'); // mengisi judul pertama
		    $objPHPExcel->getActiveSheet()->setCellValue('F1', 'KET'); // mengisi judul kedua
/*		    $objPHPExcel->getActiveSheet()->setCellValue('E5', ': '.$district); // mengisi judul kedua
...
//			$objPHPExcel->getActiveSheet()->getStyle('A'.$start.':S'.($num+3))->applyFromArray($styleAll); // set style untuk area A5 sampai M(sebanyak berapa baris datanya)
			...
			$file_name = $v->kta_propinsi_id.'-'.$v->propinsi_nama.'.xlsx';
		}
...
/*		            ->setCellValue('G'.$start_row, date('n', strtotime($v->kta_tgl_lahir)))
		            ...
		            ->setCellValue('K'.$start_row, $v->kel_nama)
