...
class Status_kartu extends AdminController {  
	function __construct()    
	{
		...
		$this->_set_title('Laporan Status Keaktifan Anggota');
	...
	function index(){
		...
		if($this->jCfg['page_tab']==2){
			...
			$this->data_table = $this->M->status_nonaktif($par_filter);
			...
			$this->data_table = $this->M->status_aktif($par_filter);
		}
	...
	function collective(){
		if(isset($_POST['item']) && count($_POST['item'])>0){
			...
			$this->_v($this->folder_view.$this->prefix_view."_collective",$data);
		}else{
	...
	function save_collective(){
		...
		if(isset($_POST['item']) && count($_POST['item'])>0){
			...
			$this->DATA->table = "app_bayar_kolektif";
			$this->db->insert("app_bayar_kolektif",array(
					"kol_date"		=> date("Y-m-d H:i:s"),
					"kol_pay_type"	=> $this->input->post('kta_jenis_bayar'),
					"kol_nominal"	=> $this->input->post('kta_jumlah_bayar'),
					"kol_note"		=> $this->input->post('kta_note_bayar')
				));
			$id_kol = $this->db->insert_id();
			...
			foreach ((array)$_POST['item'] as $k => $v) {
				$this->DATA->table = "app_kta";
				$data = array(
					'kta_jumlah_bayar'	=> (int)$this->input->post('kta_jumlah_bayar')/count($this->input->post('item')),
					'kta_lunas'			=> 1,
				...
				$this->db->insert("app_bayar_kolektif_item",array(
						"ki_kol_id"		=> $id_kol,
						"ki_kartu_id"	=> $a['id'],
						"ki_nomor_kartu"=> $nomor_kta
					));
