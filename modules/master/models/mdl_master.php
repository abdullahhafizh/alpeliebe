...
class mdl_master extends CI_Model{ 
	...
	function kta($p=array(),$count=FALSE){
		...
		$this->db->select('DISTINCT(kta_id) as id, app_kta.*,app_agama.agama_nama,app_propinsi.propinsi_nama,app_kabupaten.kab_nama,app_kecamatan.kec_nama,app_kelurahan.kel_nama,app_jabatan.jabatan_nama, app_pekerjaan.pekerjaan_nama, app_negara.negara_nama');		
...
*/
		...
		$this->db->join('app_jabatan','app_jabatan.jabatan_id=app_kta.kta_jabatan','LEFT');
		$this->db->join('app_pekerjaan','app_pekerjaan.pekerjaan_id=app_kta.kta_pekerjaan','LEFT');
		$this->db->join('app_negara','app_propinsi.negara_kode=app_negara.negara_kode','LEFT');
/*		
...
//		$this->db->where('app_topup.penggunaID', $this->jCfg['user']['penggunaid']);
	...
	function upload_kta($p=array(),$count=FALSE){
		...
		$this->db->select('DISTINCT(kta_id) as id,app_kta.kta_id, app_kta.kta_nama_lengkap, app_kta.time_add, app_kta.kta_email, app_kta.kta_hp, app_kta.kta_status_data');		
...
//		}
	...
	function kta_list($p=array(),$count=FALSE){		
		...
		$this->db->select('DISTINCT(kta_id) as id, app_kta.kta_nomor_kartu, app_kta.kta_id, app_kta.kta_nama_lengkap, app_kta.kta_alamat, app_kta.kta_foto_wajah, app_kta.kta_status_data, app_kta.is_cetak, app_negara.negara_nama, app_kta.kta_divisi,
		app_propinsi.propinsi_nama,app_kabupaten.kab_nama,app_jabatan.jabatan_nama, app_user.user_fullname');		
		$this->db->join('app_propinsi','app_propinsi.propinsi_kode=app_kta.kta_propinsi','LEFT');
		$this->db->join('app_negara','app_propinsi.negara_kode=app_negara.negara_kode','LEFT');
		$this->db->join('app_kabupaten','app_kabupaten.kab_kode=app_kta.kta_kabupaten','LEFT');
		$this->db->join('app_jabatan','app_jabatan.jabatan_id=app_kta.kta_jabatan','LEFT');
		...
		$this->db->order_by('app_kta.time_add','DESC');
		$qry = $this->db->get('app_kta');
		if($count==FALSE){
			$total = $this->kta_list($p,TRUE);
			return array(
					"data"	=> $qry->result(),
					"total" => $total
				);
...
function biaya($p=array(),$count=FALSE){
		
		$total = 0;

		$this->db->select("app_biaya_kirim.*,app_propinsi.propinsi_nama");
		$this->db->join("app_propinsi","app_propinsi.propinsi_kode = app_biaya_kirim.bk_tujuan");
		// dont modified....
		if( trim($this->jCfg['search']['colum'])=="" && trim($this->jCfg['search']['keyword']) != "" ){
			$str_like = "( ";
			$i=0;
		...
		$qry = $this->db->get("app_biaya_kirim");
		if($count==FALSE){
			$total = $this->biaya($p,TRUE);
			return array(
					"data"	=> $qry->result(),
					"total" => $total
				);
