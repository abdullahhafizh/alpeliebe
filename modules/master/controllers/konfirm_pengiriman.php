...
class Konfirm_pengiriman extends AdminController {  
	function __construct()    
	{
		...
		$this->_set_title('Pengiriman Order Kartu');
	...
	function save(){
		...
		$data = array(
			'order_send_date'	=> date("Y-m-d H:i:s"),
			'order_resi'		=> $this->input->post('resi'),
			'order_status'		=> 2,
		);	
		...
		redirect($this->own_link."?msg=".urldecode('Update Nomor Resi Order Berhasil')."&type_msg=success");
	}
	function export(){
			$par_filter = array(
				"tab_lunas"	=> $this->jCfg['page_tab'],
