		...
		if($get_max->j ==  0){
			...
			$data = array(
				...
				array(
					...
					'param'		=> array(
									'nama' => "_".$this->input->post('kta_pemesan')."_".$this->input->post('kta_no_ktp')."_suket", 
									'field' => 'kta_lampiran3', 
									'par'	=> array('kta_id' => $id)
								)
				));
...
//		debugCode($list);
		foreach ($list as $customers) {
			...
			$row = array();
			$row[] = $no;
			$row[] = $customers->kta_nama_lengkap;
			$row[] = $customers->kta_propinsi;
			$row[] = $customers->kta_kabupaten;
			$row[] = $customers->kta_kecamatan;
			$row[] = $customers->kta_kelurahan;
			$row[] = $customers->kta_status_data;
			$data[] = $row;
		}
//			debugCode($list['data']);
		$output = array(
